...
{
	print("|----------------------------------|");
	print("| Pesona Roleplay | A4-RC-Version |");
	print("|----------------------------------|");
}
...
//	CreateHBEProgressBarTD(playerid);
	...
	CreateGovTD(playerid);

	CreateGudangTD(playerid);

	CreateTransaksi(playerid);

	CreateTransaksi2(playerid);
	...
	else if(checkpointid == pTutorialRCP[playerid] && IsPlayerInDynamicRaceCP(playerid, pTutorialRCP[playerid]))
	{
		switch(pTutorialStep[playerid])
		{
			...
			case 3:
			{
				...
				SendClientMessage(playerid, X11_YELLOW, "[Tutorial] Inilah salah satu "CYAN"GYM "YELLOW"yang ada di server Pesona, dekati alat dan tekan "RED"tombol 'Y'.");
				SendClientMessage(playerid, X11_YELLOW, "------ # Akhir Informasi ------");
			...
			case 12: //carnival
			{
				...
				SendClientMessage(playerid, X11_YELLOW, "[Tutorial] Tidak hanya memancing, kamu juga dapat berburu rusa di Pesona. Beli alat terlebih dahulu di "CYAN"Warung.");
	...
	if(!ispassenger)
	{
		...
		if(IsLSPDVehicle(vehicleid))
		{
			if(AccountData[playerid][pFaction] != FACTION_LSPD)
			{
				...
				ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Kepolisian Pesona!");
			}
		}
		...
		if(IsLSFDVehicle(vehicleid))
		{
			if(AccountData[playerid][pFaction] != FACTION_LSFD)
			{
				...
				GetPlayerPos(playerid, slx, sly, slz);
				SetPlayerPos(playerid, slx, sly, slz + 3.5);
				ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Paramedis Pesona!");
			}
		}
		...
		if(IsSAGOVVehicle(vehicleid))
		{
			if(AccountData[playerid][pFaction] != FACTION_SAGOV)
			{
				...
				GetPlayerPos(playerid, slx, sly, slz);
				SetPlayerPos(playerid, slx, sly, slz + 3.5);
				ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Pemerintah Kota Pesona!");
			}
		}
		...
		if(IsFOX11LAVehicle(vehicleid))
		{
			if(AccountData[playerid][pFaction] != FACTION_FOX11)
			{
				...
				GetPlayerPos(playerid, slx, sly, slz);
				SetPlayerPos(playerid, slx, sly, slz + 3.5);
				ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Pewarta Pesona!");
			}
		}
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if(newstate == PLAYER_STATE_DRIVER) //naik
	{
		...
		if(IsAMowingSidejobVeh(SavingVehID[playerid]))
		{
            ...
            {
                Dialog_Show(playerid, "SidejobMowing", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Sidejob Mowing", "Apakah anda ingin memulai tugas mowing?\nAnda akan menerima bayaran berdasarkan jumlah rumput yang anda potong.", "Yes", "No");
            }
		}
		...
		if(IsASweeperSidejobVeh(SavingVehID[playerid]))
		{
            ...
            {
                Dialog_Show(playerid, "SidejobSweeper", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Sidejob Sweeper", "Apakah anda ingin memulai tugas sweeper?\nAnda akan menerima bayaran setelah membersihkan jalanan.", "Yes", "No");
            }
		}
		...
		if(IsAForkliftSidejobVeh(SavingVehID[playerid]))
		{
            ...
            {
                Dialog_Show(playerid, "SidejobForklift", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Sidejob Forklift", "Apakah anda ingin memulai tugas forklift?\nAnda akan menerima bayaran setelah membongkar muat 10 crates.", "Yes", "No");
            }
		}
		...
		if(IsATrashCollectorSidejobVeh(SavingVehID[playerid]))
		{
            ...
            {
                Dialog_Show(playerid, "SidejobTrashCollector", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Sidejob Trash Collector", "Apakah anda ingin memulai tugas trash collector?\nAnda akan menerima bayaran setelah mengangkut 25 trash.", "Yes", "No");
            }
		}
		...
		if(SavingVehID[playerid] == g_CarstealCarPhysic[playerid])
		{
			if(g_CarstealCarPhysic[playerid] != INVALID_VEHICLE_ID)
			{
				...
				ShowFivemNotify(playerid, "Pesona Premiere~n~CAR STEAL", "Anda telah menemukan kendaraannya, kembalikan kepada gangster", "hud:radar_qmark", 25);
				...
				if(!g_CarstealCarFound[playerid])
				{
					...
					{
						ShowFivemNotify(i, "Pesona Premiere~n~CAR STEAL", sprintf("Pencurian kendaraan %s berlangsung di %s", GetVehicleModelName(GetVehicleModel(g_CarstealCarPhysic[playerid])), GetLocation(crstlX, crstlY, crstlZ)), "hud:radar_qmark", 25);
...
OnPlayerUseItem(playerid, const name[])
{
	...
	else if(!strcmp(name, "Karung Goni"))
	{
		...
		if(count == 0) 
		{
			...
			return Dialog_Show(playerid, "UnusedDialog", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Karung Goni", "Tidak ada pemain terdekat!", "Tutup", "");
		}

		Dialog_Show(playerid, "Blindfold", DIALOG_STYLE_LIST, ""NEXOTIS"Pesona "WHITE"- Karung Goni", frmxt, "Pilih", "Batal");
	}
	...
	else if(!strcmp(name, "Disposable Phone"))
	{
		...
		Dialog_Show(playerid, "DisposPhoneCatalog", DIALOG_STYLE_TABLIST_HEADERS, ""NEXOTIS"Pesona "WHITE"- Order Gun & Equipment",
        "Item\tPrice\n\
		Colt-45\t$3,000\n\
	...
	else if(!strcmp(name, "Pilox"))
	{
		...
		Dialog_Show(playerid, "GraffitiAdd", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Add Graffiti",
	...
	else if(!strcmp(name, "Smartphone"))
	{
		...
		else
		{
			...
			else
			{
				...
				TextDrawShowForPlayer(playerid, LockScreenTD[6]);
				TextDrawShowForPlayer(playerid, LockScreenTD[7]);
				...
				TextDrawShowForPlayer(playerid, LockScreenTD[11]);
				TextDrawShowForPlayer(playerid, LockScreenTD[12]);
				TextDrawShowForPlayer(playerid, LockScreenTD[13]);
				TextDrawShowForPlayer(playerid, LockScreenTD[14]);
				TextDrawShowForPlayer(playerid, LockScreenTD[15]);				

				TextDrawShowForPlayer(playerid, HomeButtonPhone);
	...
	else if(!strcmp(name, "Changename Card"))
	{
		...
		Dialog_Show(playerid, "ChangeName", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Change Name",
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
	...
	//td elektronik	
	else if(clickedid == ElektronikTD[19])
	{
		PlayerPlaySound(playerid, 21001, 0.0, 0.0, 0.0);

		CancelSelectTextDraw(playerid);
		HideElektronik(playerid);
	...
	else if(clickedid == ElektronikTD[2])
	{
		PlayerPlaySound(playerid, 21001, 0.0, 0.0, 0.0);
		...
		ShowItemBox(playerid, "Cash", "Removed $1,500x", 1212, 4);
		ShowItemBox(playerid, "Smartphone", "Received 1x", 18867, 5);
		ShowTDN(playerid, NOTIFICATION_SUCCESS, "Transaksi berhasil dilakukan.");
		HideElektronik(playerid);
	}
	else if(clickedid == ElektronikTD[3])
	{
		PlayerPlaySound(playerid, 21001, 0.0, 0.0, 0.0);
		...
		static Float:countingtotalweight;
		countingtotalweight = GetTotalWeightFloat(playerid) + float(GetItemWeight("Radio"))/1000;
		if(countingtotalweight > GetPlayerInventoryWeight(playerid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Inventory anda sudah penuh!");
	...
	else if(clickedid == ElektronikTD[4])
	{
		PlayerPlaySound(playerid, 21001, 0.0, 0.0, 0.0);
		if(AccountData[playerid][pMoney] < 500) return ShowTDN(playerid, NOTIFICATION_ERROR, "Uang anda tidak cukup!");
	...
	else if(clickedid == ElektronikTD[5])
	{
		PlayerPlaySound(playerid, 21001, 0.0, 0.0, 0.0);
		if(AccountData[playerid][pVIP] < 1) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan Donatur VIP!");
			
		if(AccountData[playerid][pMoney] < 3000) return ShowTDN(playerid, NOTIFICATION_ERROR, "Uang anda tidak cukup!");

		TakePlayerMoneyEx(playerid, 3000);
		AccountData[playerid][pBoombox] = true;

		ShowItemBox(playerid, "Cash", "Removed $3,000x", 1212, 4);
		ShowItemBox(playerid, "Boombox", "Received 1x", 2103, 5);

		ShowTDN(playerid, NOTIFICATION_SUCCESS, "Transaksi berhasil dilakukan.");
		HideElektronik(playerid);
	}
	else if(clickedid == ElektronikTD[6])
	{
		PlayerPlaySound(playerid, 21001, 0.0, 0.0, 0.0);
		if(AccountData[playerid][pVIP] < 2) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan Donatur VIP Super Pinky!");
	...
	//job center
	else if(clickedid == JobCenterTD[1])
	{
		HideJobCenterTD(playerid);
	}

	else if(clickedid == JobCenterTD[4]) //farmer
	{
		HideJobCenterTD(playerid);
	...
	else if(clickedid == JobCenterTD[8]) //miner
	{
		HideJobCenterTD(playerid);
	...
	else if(clickedid == JobCenterTD[12]) //butcher
	{
		HideJobCenterTD(playerid);
	...
	else if(clickedid == JobCenterTD[16]) //oilman
	{
		HideJobCenterTD(playerid);
	...
	else if(clickedid == JobCenterTD[20]) //angkot driver
	{
		HideJobCenterTD(playerid);
	...
	else if(clickedid == JobCenterTD[24]) //fisherman
	{
		HideJobCenterTD(playerid);
	...
	else if(clickedid == JobCenterTD[28]) //kargo
	{
		HideJobCenterTD(playerid);
	...
	else if(clickedid == JobCenterTD[31]) //Next Job
	{
		HideJobCenterTD(playerid);
		ShowJobCenterTD2(playerid);
	}
	//job center 2
	else if(clickedid == JobCenterTD2[1])
	{
		//HideJobCenterTD(playerid);
		HideJobCenterTD2(playerid);
	}		
	else if(clickedid == JobCenterTD2[19]) //porter
	{
		HideJobCenterTD2(playerid);
	...
	else if(clickedid == JobCenterTD2[16]) //porter
	{
		HideJobCenterTD2(playerid);
	...
	else if(clickedid == JobCenterTD2[13]) //kayu
	{
		HideJobCenterTD2(playerid);
	...
	else if(clickedid == JobCenterTD2[11]) //pelaut
	{
		HideJobCenterTD2(playerid);
	...
	else if(clickedid == JobCenterTD2[9]) //peternak
	{
		HideJobCenterTD2(playerid);
	...
	else if(clickedid == JobCenterTD2[7]) //penjahit
	{
		HideJobCenterTD2(playerid);
	...
	else if(clickedid == JobCenterTD2[5]) //pengangguran
	{
		HideJobCenterTD2(playerid);
	...
	else if(clickedid == JobCenterTD2[36]) //sawit
	{
		HideJobCenterTD2(playerid);
		AccountData[playerid][pJob] = JOB_PALM;
		RefreshMapJob(playerid);
		ShowTDN(playerid, NOTIFICATION_SUCCESS, sprintf("Anda berhasil memutuskan untuk menjadi ~p~%s.", GetJobName(playerid)));
	}	
	else if(clickedid == JobCenterTD2[34]) //Kembali Ke job 1
	{
	...
	//td rental by vyn
	else if(clickedid == ui_rental[15]) //batal
	{
		HideRentaLTD(playerid);
		CancelSelectTextDraw(playerid);
	}
	else if(clickedid == ui_rental[8]) //fagio
	{
		ShowTransaksi(playerid);
		HideRentaLTD(playerid);
		SelectTextDraw(playerid, 0xff91a4cc);
	}
	else if(clickedid == ui_rental[10]) //watfarer
	{
		ShowTransaksi2(playerid);
		HideRentaLTD(playerid);
		SelectTextDraw(playerid, 0xff91a4cc);
	}			
	else if(clickedid == StarterpackTD[24]) //watfarer
	{
	...
	else if(clickedid == InventTD[5]) //tombol give item inventory
	{
		...
		{
			PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
			return Dialog_Show(playerid, "UnusedDialog", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Give Item", "Tidak ada pemain terdekat!", "Tutup", "");
		}

		Dialog_Show(playerid, "InventoryGive", DIALOG_STYLE_LIST, ""NEXOTIS"Pesona "WHITE"- Give Item", frmxt, "Pilih", "Batal");
	...
	else if(clickedid == InventTD[7]) //tombol drop item inventory
	{
		...
		if(gid == -1) //jika tidak dekat dengan tong sampah
		{
			...
			if(IsPlayerInRangeOfPoint(playerid, 3.5, 2341.185,-2071.370,13.546))//2341.185,-2071.370,13.546
			{
				if(!strcmp(string, "Minyak Sawit", false))
				{
					...
					ShowItemBox(playerid, string, sprintf("Removed %dx", AccountData[playerid][pItemQuantity]), 19820, 4);
					ShowItemBox(playerid, "Cash", sprintf("Received $%sx", FormatMoney(rumuspendapatan)), 1212, 5);
	...
	else if(clickedid == ATMTD[13]) //logout ATM
	{
	...
	else if(clickedid == ATMTD[19]) //withdraw ATM
	{
		if(!AVC_PConnected[playerid]) return Kick(playerid);
		Dialog_Show(playerid, "ATMWithdraw", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Withdraw",
		"Mohon masukkan berapa jumlah yang ingin ditarik:", "Tarik", "Batal");
	}
	else if(clickedid == ATMTD[21]) //deposit ATM
	{
		if(!AVC_PConnected[playerid]) return Kick(playerid);
		Dialog_Show(playerid, "ATMDeposit", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Deposit",
		"Mohon masukkan berapa jumlah yang ingin disimpan:", "Depo", "Batal");
	}
	else if(clickedid == ATMTD[20]) //transfer ATM
	{
		if(!AVC_PConnected[playerid]) return Kick(playerid);
		Dialog_Show(playerid, "ATMTransfer", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Transfer",
		"Mohon masukkan nomor rekening tujuan:", "Set", "Batal");
	}
	else if(clickedid == ATMTD[35]) //quick withdraw 10
	{
		if(!AVC_PConnected[playerid]) return Kick(playerid);
		AccountData[playerid][pTempValue2] = 1;
		Dialog_Show(playerid, "ATMQuickMenu", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Quick Withdraw", ""WHITE"Apakah anda yakin ingin menarik uang senilai "GREEN"$10,000 "WHITE"dari saldo rekening?\nPilih "YELLOW"'lanjut' "WHITE"untuk melanjutkan transaksi ini!", "Lanjut", "Batal");
	}
	else if(clickedid == ATMTD[36]) //quick withdraw 50
	{
		if(!AVC_PConnected[playerid]) return Kick(playerid);
		AccountData[playerid][pTempValue2] = 2;
		Dialog_Show(playerid, "ATMQuickMenu", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Quick Withdraw", ""WHITE"Apakah anda yakin ingin menarik uang senilai "GREEN"$50,000 "WHITE"dari saldo rekening?\nPilih "YELLOW"'lanjut' "WHITE"untuk melanjutkan transaksi ini!", "Lanjut", "Batal");
	}
	else if(clickedid == ATMTD[38]) //quick withdraw 25
	{
		if(!AVC_PConnected[playerid]) return Kick(playerid);
		AccountData[playerid][pTempValue2] = 3;
		Dialog_Show(playerid, "ATMQuickMenu", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Quick Withdraw", ""WHITE"Apakah anda yakin ingin menarik uang senilai "GREEN"$250,000 "WHITE"dari saldo rekening?\nPilih "YELLOW"'lanjut' "WHITE"untuk melanjutkan transaksi ini!", "Lanjut", "Batal");
	}
	else if(clickedid == ATMTD[37]) //quick withdraw 500
	{
		AccountData[playerid][pTempValue2] = 4;
		Dialog_Show(playerid, "ATMQuickMenu", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Quick Withdraw", ""WHITE"Apakah anda yakin ingin menarik uang senilai "GREEN"$500,000 "WHITE"dari saldo rekening?\nPilih "YELLOW"'lanjut' "WHITE"untuk melanjutkan transaksi ini!", "Lanjut", "Batal");
	}

	else if(clickedid == ATMTD[39]) //quick deposit 10
	{
		if(!AVC_PConnected[playerid]) return Kick(playerid);
		AccountData[playerid][pTempValue2] = 5;
		Dialog_Show(playerid, "ATMQuickMenu", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Quick Deposit", ""WHITE"Apakah anda yakin ingin deposit uang senilai "GREEN"$10,000 "WHITE"ke saldo rekening?\nPilih "YELLOW"'lanjut' "WHITE"untuk melanjutkan transaksi ini!", "Lanjut", "Batal");
	}
	else if(clickedid == ATMTD[40]) //quick deposit 50
	{
		if(!AVC_PConnected[playerid]) return Kick(playerid);
		AccountData[playerid][pTempValue2] = 6;
		Dialog_Show(playerid, "ATMQuickMenu", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Quick Deposit", ""WHITE"Apakah anda yakin ingin deposit uang senilai "GREEN"$50,000 "WHITE"ke saldo rekening?\nPilih "YELLOW"'lanjut' "WHITE"untuk melanjutkan transaksi ini!", "Lanjut", "Batal");
	}
	else if(clickedid == ATMTD[42]) //quick deposit 25
	{
		if(!AVC_PConnected[playerid]) return Kick(playerid);
		AccountData[playerid][pTempValue2] = 7;
		Dialog_Show(playerid, "ATMQuickMenu", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Quick Deposit", ""WHITE"Apakah anda yakin ingin deposit uang senilai "GREEN"$250,000 "WHITE"ke saldo rekening?\nPilih "YELLOW"'lanjut' "WHITE"untuk melanjutkan transaksi ini!", "Lanjut", "Batal");
	}
	else if(clickedid == ATMTD[41]) //quick deposit 500
	{
		AccountData[playerid][pTempValue2] = 8;
		Dialog_Show(playerid, "ATMQuickMenu", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Quick Deposit", ""WHITE"Apakah anda yakin ingin deposit uang senilai "GREEN"$500,000 "WHITE"ke saldo rekening?\nPilih "YELLOW"'lanjut' "WHITE"untuk melanjutkan transaksi ini!", "Lanjut", "Batal");
	}
	else if(clickedid == RadialTD[1]) //exit radial main menu
	{
		PlayerPlaySound(playerid, 21001, 0.0, 0.0, 0.0);

		CancelSelectTextDraw(playerid);
		HideRadialTD(playerid);
		HideRadialTD2(playerid);
	...
	else if(clickedid == RadialTD[2]) //HP
	{
	...
	else if(clickedid == RadialTD[0]) //Inventory
	{
		if(AccountData[playerid][pActivityTime] != 0) return ShowTDN(playerid, NOTIFICATION_WARNING, "Anda sedang melakukan sesuatu, mohon tunggu progress bar selesai!");
		if(AccountData[playerid][pBlindfolded]) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak dapat melakukannya saat ini!");
	...
	else if(clickedid == RadialTD[5]) //fashion
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
	...
	else if(clickedid == RadialTD[4]) //radial next
	{
		CancelSelectTextDraw(playerid);
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		//HideRadialTD(playerid);
		ShowRadialTD2(playerid);
	}
	else if(clickedid == RadialTD[6]) //EMOTTD
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		HideRadialTD(playerid);
		HideRadialTD2(playerid);
	...
	else if(clickedid == RadialTD[3]) //invoice
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		...
		if(count == 0)
		{
			Dialog_Show(playerid, "UnusedDialog", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Unpaid Invoice",
		...
		else
		{
			...
			Dialog_Show(playerid, "InvoicePay", DIALOG_STYLE_TABLIST_HEADERS, ""NEXOTIS"Pesona "WHITE"- Unpaid Invoice",
			xjjs, "Bayar", "Batal");
		}
	}
	//radialtd yang ke 2
	else if(clickedid == RadialTD2[0]) //kembali ke 1
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
	...
	else if(clickedid == RadialTD2[6]) //kendaraan
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		HideRadialTD(playerid);
		HideRadialTD2(playerid);
	...
	else if(clickedid == RadialTD2[5]) //ktp/lisensi
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		HideRadialTD(playerid);
		HideRadialTD2(playerid);
	...
	else if(clickedid == RadialTD2[4]) //dokument
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		HideRadialTD(playerid);
		HideRadialTD2(playerid);
		...
		Dialog_Show(playerid, "MyDocuments", DIALOG_STYLE_LIST, ""NEXOTIS"Pesona "WHITE"- Dokumen",
	...
	else if(clickedid == RadialFashionTD[23]) //helmet/topi
	{
		...
		Dialog_Show(playerid, "ToysEdit", DIALOG_STYLE_TABLIST_HEADERS, ""NEXOTIS"Pesona "WHITE"- Edit Fashion (Hat/Helmet)", string, "Pilih", "Kembali");
	}
	else if(clickedid == RadialFashionTD[24]) //kacamata
	{
		...
		Dialog_Show(playerid, "ToysEdit", DIALOG_STYLE_TABLIST_HEADERS, ""NEXOTIS"Pesona "WHITE"- Edit Fashion (Kacamata)", string, "Pilih", "Kembali");
	}
	else if(clickedid == RadialFashionTD[25]) //aksesoris
	{
		...
		Dialog_Show(playerid, "ToysEdit", DIALOG_STYLE_TABLIST_HEADERS, ""NEXOTIS"Pesona "WHITE"- Edit Fashion (Aksesoris)", string, "Pilih", "Kembali");
	}
	else if(clickedid == RadialFashionTD[26]) //tas/koper
	{
		...
		Dialog_Show(playerid, "ToysEdit", DIALOG_STYLE_TABLIST_HEADERS, ""NEXOTIS"Pesona "WHITE"- Edit Fashion (Tas/Koper)", string, "Pilih", "Kembali");
	}
	//--------- Kendaraan --------
	else if(clickedid == RadialVehTD[22]) //exit
	{
		PlayerPlaySound(playerid, 21001, 0.0, 0.0, 0.0);
		HideRadialVehTD(playerid);
		ShowRadialTD(playerid);
	}
	else if(clickedid == RadialVehTD[12]) //kunci
	{
		ShowServerNameTD(playerid);
	...
	else if(clickedid == RadialVehTD[10]) //lampu
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		CancelSelectTextDraw(playerid);
		HideRadialVehTD(playerid);
	...
	else if(clickedid == RadialVehTD[13]) //bagasi
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		CancelSelectTextDraw(playerid);
		HideRadialVehTD(playerid);
		...
		foreach(new carid : PvtVehicles)
		{
			if(vehid == PlayerVehicle[carid][pVehPhysic])
			{
				...
				Dialog_Show(playerid, "VehicleStorage", DIALOG_STYLE_LIST, sprintf("Bagasi Kendaraan "YELLOW"%s "NEXOTIS"- %s", GetVehicleModelName(PlayerVehicle[carid][pVehModelID]), PlayerVehicle[carid][pVehPlate]), "Simpan Barang\nAmbil Barang", "Pilih", "Batal");
			}
		}
	}
	else if(clickedid == RadialVehTD[14]) //holster
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		CancelSelectTextDraw(playerid);
		HideRadialVehTD(playerid);
	...
	else if(clickedid == RadialVehTD[15]) //trunk
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		CancelSelectTextDraw(playerid);
		HideRadialVehTD(playerid);
		...
		Dialog_Show(playerid, "VehicleTrunk", DIALOG_STYLE_LIST, ""NEXOTIS"Pesona "WHITE"- Trunk",
	...
	else if(clickedid == RadialVehTD[11]) //hood
	{
		PlayerPlaySound(playerid, 21000, 0.0, 0.0, 0.0);
		CancelSelectTextDraw(playerid);
		HideRadialVehTD(playerid);
	...
	else if(clickedid == RadialCardTD[25]) //lihat KTP
	{
		...
		{
			Dialog_Show(playerid, "UnusedDialog", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Kartu Tanda Penduduk",
	...
	else if(clickedid == RadialCardTD[22]) //tunjuk KTP
	{
        ...
        if(count > 0)
		{
            Dialog_Show(playerid, "ShowToIDCard", DIALOG_STYLE_LIST, ""NEXOTIS"Pesona "WHITE"- Tunjukkan KTP",
			frmxt, "Pilih", "Batal");
		}
	}
	else if(clickedid == RadialCardTD[23]) //lihat lisensi
	{
		...
		{
			Dialog_Show(playerid, "UnusedDialog", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Licenses",
	...
	else if(clickedid == RadialCardTD[24]) //tunjuk lisensi
	{
        ...
        if(count > 0)
		{
            Dialog_Show(playerid, "ShowToLicense", DIALOG_STYLE_LIST, ""NEXOTIS"Pesona "WHITE"- Tunjukkan Lisensi",
			frmxt, "Pilih", "Batal");
		}
	}
	else if(clickedid == TollTD[24])
	{
		...
		{
			if(AccountData[playerid][pMoney] < 100) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak memuliki cukup uang untuk membayar tol! ($80)");

			TakePlayerMoneyEx(playerid, 100);
			ShowItemBox(playerid, "Cash", "Removed $100", 1212, 5);
			ShowTDN(playerid, NOTIFICATION_INFO, "Anda telah membayar biaya tol sebesar ~g~$100.");
		}
...
public OnDynamicPlayerTextdrawClicked(playerid, PlayerText:playertextid)
{
	...
	if(playertextid == InventWeightTD[playerid][3]) //tombol set jumlah item inventory yang akan diberi
	{
		...
		Dialog_Show(playerid, "InventorySetValue", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Set Value", "Mohon masukkan jumlah item yang akan diberikan:", "Set", "Batal");
		PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
	}
	//td rental fagio
	if(playertextid == TransaksiFaggio[playerid][14]) // Close/tidakjadi
	{
        HideTransaksi(playerid);
		CancelSelectTextDraw(playerid);	
		return 1;
	}
	if(playertextid == TransaksiFaggio[playerid][2]) // sewa fagio
	{
        ...
        VehicleRental_Create(playerid, RentalData[brid][Model][0], 1200, brid, RentalData[brid][SpawnPos][0], RentalData[brid][SpawnPos][1], RentalData[brid][SpawnPos][2], RentalData[brid][SpawnPos][3], color1, color2, RentalData[brid][World], RentalData[brid][Interior]);
        HideTransaksi(playerid);
		return 1;
	}
	if(playertextid == TransaksiFaggio[playerid][3]) // bank fagio
	{
		new brid = AccountData[playerid][pInRent],
        color1 = RandomEx(0, 255),
        color2 = RandomEx(0, 255),
        ...
        if(AccountData[playerid][pBankMoney] < RentalData[brid][Cost][0])
    	return ShowTDN(playerid, NOTIFICATION_ERROR, "Saldo bank anda tidak cukup!");
        if(count >= GetPlayerVehicleLimit(playerid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda telah mencapai batas slot maksimum kendaraan!");
            
        AccountData[playerid][pBankMoney] -= RentalData[brid][Cost][0];
        VehicleRental_Create(playerid, RentalData[brid][Model][0], 1200, brid, RentalData[brid][SpawnPos][0], RentalData[brid][SpawnPos][1], RentalData[brid][SpawnPos][2], RentalData[brid][SpawnPos][3], color1, color2, RentalData[brid][World], RentalData[brid][Interior]);
        HideTransaksi(playerid);
		return 1;
    }	
	//td rental wayfarer
	if(playertextid == TransaksiWayfarer[playerid][14]) // Close/tidakjadi
	{
        HideTransaksi2(playerid);
		CancelSelectTextDraw(playerid);	
		return 1;
	}
	if(playertextid == TransaksiWayfarer[playerid][2]) // sewa wayfarer
	{
	    new brid = AccountData[playerid][pInRent],
        color1 = RandomEx(0, 255),
        color2 = RandomEx(0, 255),
        ...
        AccountData[playerid][pBankMoney] -= RentalData[brid][Cost][0];     
        VehicleRental_Create(playerid, RentalData[brid][Model][1], 1200, brid, RentalData[brid][SpawnPos][0], RentalData[brid][SpawnPos][1], RentalData[brid][SpawnPos][2], RentalData[brid][SpawnPos][3], color1, color2, RentalData[brid][World], RentalData[brid][Interior]);
        HideTransaksi2(playerid);
		return 1;
	}
	if(playertextid == TransaksiWayfarer[playerid][3]) // bank wayfarer
	{
	    new brid = AccountData[playerid][pInRent],
        color1 = RandomEx(0, 255),
        color2 = RandomEx(0, 255),
	...
	if(playertextid == GudangTD[playerid][8]) // Close
	{
		HideGudangTD(playerid);
		ShowHBETD(playerid);
		ShowStatsTD(playerid);				
	    ShowServerNameTD(playerid);	
		CancelSelectTextDraw(playerid);	
	...
	if(playertextid == GudangTD[playerid][3]) // Sewa Gudang
	{
		HideGudangTD(playerid);
		ShowHBETD(playerid);
		ShowStatsTD(playerid);
		Gudang_Rental(playerid);
		CancelSelectTextDraw(playerid);
	    ShowServerNameTD(playerid);		
		return 1;
	}
	if(playertextid == GudangTD[playerid][5]) // Berhenti Sewa
	{
		HideGudangTD(playerid);
		ShowHBETD(playerid);
		ShowStatsTD(playerid);
		Gudang_Unrental(playerid);
	    ShowServerNameTD(playerid);
		return 1;
	}						
	if(playertextid == NEXOTIS_GovTD[playerid][8]) // Close
	{
		HideGovTD(playerid);
		ShowHBETD(playerid);
		ShowStatsTD(playerid);				
	...
	if(playertextid == NEXOTIS_GovTD[playerid][3]) // Ambil Pekerjaan
	{
		HideGovTD(playerid);
		ShowHBETD(playerid);
		ShowStatsTD(playerid);
	...
	if(playertextid == NEXOTIS_GovTD[playerid][5]) // Buat KTP
	{
		HideGovTD(playerid);
		ShowHBETD(playerid);
		ShowStatsTD(playerid);
	    ShowServerNameTD(playerid);
		Disnaker_Select(playerid);
		return 1;
	}	
	else if(playertextid == pJobMixTD[playerid][5])
	{
		Dialog_Show(playerid, "SetSemenValue", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Batching Plant", "Mohon masukkan jumlah agregat yang akan ditetapkan:", "Set", "Batal");
		PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
	}
	else if(playertextid == pJobMixTD[playerid][6])
	{
		Dialog_Show(playerid, "SetPasirValue", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Batching Plant", "Mohon masukkan jumlah agregat yang akan ditetapkan:", "Set", "Batal");
		PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
	}
	else if(playertextid == pJobMixTD[playerid][7])
	{
		Dialog_Show(playerid, "SetKrikilAValue", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Batching Plant", "Mohon masukkan jumlah agregat yang akan ditetapkan:", "Set", "Batal");
		PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
	}
	else if(playertextid == pJobMixTD[playerid][8])
	{
		Dialog_Show(playerid, "SetKrikilBValue", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Batching Plant", "Mohon masukkan jumlah agregat yang akan ditetapkan:", "Set", "Batal");
		PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
	}
	else if(playertextid == pJobMixTD[playerid][9])
	{
		Dialog_Show(playerid, "SetAirValue", DIALOG_STYLE_INPUT, ""NEXOTIS"Pesona "WHITE"- Batching Plant", "Mohon masukkan jumlah agregat yang akan ditetapkan:", "Set", "Batal");
		PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
	}
	return 0;
}
...
{	
	...
	else if(newkeys & KEY_CTRL_BACK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(AccountData[playerid][pActivityTime] > 0)
        {
			...
			else if(AccountData[playerid][pJob] == JOB_PALM)
			{
				if(CheckPalmTimer(playerid))
				{
					pTakePalmWoodTimer[playerid] = false;
					pWashPalmWoodTime[playerid] = false;
					pPackPalmWoodTime[playerid] = false;				
	...
	foreach(new i : Player)
	{
		...
		if(AccountData[i][pDuringCarsteal])
		{
			if(vehicleid == g_CarstealCarPhysic[i])
			{
				...
				ShowFivemNotify(i, "Pesona Premiere~n~CAR STEAL", "Kendaraan carsteal telah hancur, misi dinyatakan gagal!", "hud:radar_qmark", 25);
			}
		}
	}
	...
	if(!AccountData[playerid][pInEvent])
	{
		if(issuerid != INVALID_PLAYER_ID)
		{
			if(IsPlayerInDynamicArea(playerid, AirportGreenZone) || AccountData[playerid][pInDoor] == 41 || AccountData[playerid][pInDoor] == 44)
			{
				...
				Dialog_Show(issuerid, "UnusedDialog", DIALOG_STYLE_MSGBOX, ""NEXOTIS"Pesona "WHITE"- Anti Retard", ""RED"PERINGATAN KERAS!\n\n"YELLOW"Anda dilarang rusuh di area ini atau akan segera ditendang dari server!", "Tutup", "");
...
public FS_ChangeVoiceRadius(playerid)
{
    ...
    case 1: // Normal
    {
        PlayerTextDrawShow(playerid, RadioVoiceInfoTD[playerid][0]);
        PlayerTextDrawShow(playerid, RadioVoiceInfoTD[playerid][1]);
        PlayerTextDrawHide(playerid, RadioVoiceInfoTD[playerid][2]);
    ...
    {
        PlayerTextDrawShow(playerid, RadioVoiceInfoTD[playerid][0]);
        PlayerTextDrawHide(playerid, RadioVoiceInfoTD[playerid][1]);
        PlayerTextDrawHide(playerid, RadioVoiceInfoTD[playerid][2]);
...
CMD:setcircle(playerid, params[])
{
    if(params[0] == EOS)
	{
	    ShowTDN(playerid, NOTIFICATION_INFO, "/setcircle [playerid] [percentage]");
        //Info(playerid, "Usage: /setcircle [playerid] [percentage]");
        return 0;
    }
    ...
    if(!IsPlayerConnected(targetid)) {
        ShowTDN(playerid, NOTIFICATION_INFO, "Player not connected.");
        //Info(playerid, "Player not connected.");
        return 0;
    }

    // Validasi percentage, pastikan nilainya antara 0 dan 100
    if(percentage < 0 || percentage > 100) {
        ShowTDN(playerid,NOTIFICATION_INFO, "Invalid percentage value. Please enter a value between 0 and 100.");
        return 0;
    }
    ...
    ShowTDN(playerid,NOTIFICATION_INFO, "Circle percentage updated for the target player.");
    return 1;
}
